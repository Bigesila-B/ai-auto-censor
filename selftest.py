...
# 运行: python selftest.py   （需先准备 test.jpg 示例图，或注释掉第 3/4/5/6 节）
import base64
import http.client
import json
import os
...
# 生成合成测试图（替代本地 test.jpg，仓库不附带图片素材）
syn = np.zeros((640, 640, 3), np.uint8)
syn[:, :, 0] = 90
for yy in range(0, 640, 8):
    syn[yy, :, 1] = 160
cv2.imwrite("_test_input.jpg", syn)
src = cv2.imread("_test_input.jpg")
...
jpg = open("_test_input.jpg", "rb").read()
...
r = subprocess.run([sys.executable, "auto_censor.py", "_test_input.jpg",
                    "--mode", "solid", "--color", "00ff00", "--margin", "30"],
                   capture_output=True, text=True)
check("CLI 处理合成图正常", r.returncode == 0 and "已打码" in r.stdout,
      (r.stdout + r.stderr)[-300:])
check("输出文件已生成", cv2.imread("_test_input_censored.jpg") is not None)
...
server.shutdown()
os.remove("_test_input.jpg")
if os.path.exists("_test_input_censored.jpg"):
    os.remove("_test_input_censored.jpg")
print(f"\n===== 结果: {PASS} 通过, {FAIL} 失败 =====")
