...
# 选项:
#   --mode mosaic|blur|solid|img  打码方式（默认 mosaic 马赛克）
...
parser.add_argument("--mode", default="mosaic",
                    help="打码方式: mosaic(马赛克)/blur(高斯模糊)/solid(纯色)/img(图片遮挡)")
...
for path in args.images:
    ...
    color_desc = "纯色" if args.mode == "solid" else \
        ("马赛克" if args.mode == "mosaic" else "高斯模糊" if args.mode == "blur" else "图片遮挡")
